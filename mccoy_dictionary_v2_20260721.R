# =============================================================================
# Categorizing Written Survey Responses with a Dictionary Approach
# Source: Katharine McCoy, 2026-03-28 (converted from .Rmd to .R)
# Data: CES_25.dta, variable cps25_imp_iss (most important issue, write-in)
# =============================================================================


# ---- Packages ---------------------------------------------------------------
# install.packages(c("tidyverse", "haven", "magrittr", "tidytext", "quanteda",
#                    "tm", "SnowballC", "crayon", "progressr", "progress",
#                    "lubridate", "quanteda.textstats", "tictoc", "gtsummary",
#                    "rio", "rstudioapi"))
...
# Sets the working directory to wherever this script is saved (RStudio only).
...
ces25$cps25_imp_iss <- stringr::str_replace_all(ces25$cps25_imp_iss, "[A-Z]",
                                                ~tolower(.x))
ces25$cps25_imp_iss <- sub("^(\\w+)\\s+(\\w+)$", "\\2 \\1", ces25$cps25_imp_iss)
ces25$cps25_imp_iss <- gsub("[[:punct:]]", "", ces25$cps25_imp_iss)

# Removes spaces for some key phrases so word-level tokenization catches them
ces25$cps25_imp_iss <- str_replace_all(ces25$cps25_imp_iss,
                                       c("prime minister" = "primeminister",
...
ces25.econ <- runDictionary(   # creates a dataframe with a column counting
...
# ---- Workflow: Security / Defence and International Relations ---------------
...
# ---- Workflow: Nonanswers ---------------------------------------------------
# NOTE: the original had a missing comma after "jesaispas" (a syntax error).
dictionnaryidk <- dictionary(
  list(idk = c("99", "unsure", "dontknow", "jenesaispas", "idk",
               "prefer not", "nothing", "no", "jesaispas",
               "jaipasdereponse")))
