...
library(haven)
#load data
ces15web<-read_dta(file=here("data-raw/CES15_CPS+PES_Web_SSI Full.dta"))
#Note, if this is the case, the entire Recode script must be uncommented and run
# Rename income
ces15web$income
ces15web %>%
  rename(income_original=income,
         education_original=education,
         religion_original=religion)->ces15web
...
# #glimpse(ces15phone)
#lookfor(ces15web, "title")

# add Election
ces15web$election<-rep(2015, nrow(ces15web))
# Add Mode
ces15web$mode<-rep("Web", nrow(ces15web))

# Save the file
save(ces15web, file=here("data/ces15web.rda"))
